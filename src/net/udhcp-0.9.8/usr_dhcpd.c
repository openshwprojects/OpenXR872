...
static char dhcp_ser_interface[4] = {0};
#endif
...
#ifdef STA_SOFTAP_COEXIST
	if (!g_wlan_ap_netif) {
		DEBUG(LOG_ERR, "g_wlan_ap_netif is null, AP is not up!\n");
		goto exit_server;
	}
	sprintf(dhcp_ser_interface, "en%d", g_wlan_ap_netif->num);
	server_config.interface = dhcp_ser_interface;
	//printf("%s()%d, server_config:%p, interface:%s\n", __func__, __LINE__,
	//      &server_config, server_config.interface);
#endif
#else
	init_config();
#endif
...
#ifdef STA_SOFTAP_COEXIST
			if (!udhcpd_check_sta_connect(&stas, packet->chaddr)) {
				DEBUG(LOG_WARNING, "it's not packet to us, ignore the packet");
				continue;
			}
#endif
#endif
